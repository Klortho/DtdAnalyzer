...
<xsl:stylesheet xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <xsl:template match="attribute[attributeDeclaration[@mode='#REQUIRED']] | 
                       attribute[attributeDeclaration[@mode='#FIXED']] | 
                       attribute[attributeDeclaration[starts-with(@type, '(') and @mode!='#FIXED']]">
    <xsl:variable name="attribute" select="concat('@', @name)"/>
    <xsl:apply-templates
      select="attributeDeclaration[@mode='#REQUIRED'], 
              attributeDeclaration[@mode='#FIXED'], 
              attributeDeclaration[starts-with(@type, '(') and @mode!='#FIXED']">
    ...
    <sch:rule context="{concat(@element, '/', $attribute)}">
      <sch:assert>
        <xsl:attribute name="test">
          ...
          <xsl:value-of
            select="string-join((tokenize($values, 'or ')[position() != last()]), 'or ')"/>
        </xsl:attribute>
        ...
        <xsl:text> or</xsl:text>
        <xsl:value-of
          select="tokenize(translate(string-join((tokenize(@type, '\|')), ', '), '()', ''), ',')[last()]"/>
      ...
      <xsl:when test="attribute[attributeDeclaration/@element=$e-name]">
        <xsl:variable name="allowed-test">
          ...
          <xsl:variable name="allowed">
            <xsl:for-each-group select="attribute[attributeDeclaration/@element=$e-name]"
              group-by="@name">
        ...
        <sch:report>
          ...
          <xsl:value-of select="$element"/>
          <xsl:text> cannot contain the following attributes: </xsl:text>
          <value-of select="{concat('(', $allowed-test, ')/name()')}"/>
          <xsl:text>.</xsl:text>
        </sch:report>
      </xsl:when>
      <xsl:otherwise>
        <sch:report test="@*"><xsl:value-of select="$element"/> must not contain any
    ...
    <xsl:if test=".!='EMPTY' and not(starts-with(., '(#PCDATA)'))">
      <sch:report>
        <xsl:variable name="allowed-test">
          <xsl:text>* except (</xsl:text>
          <xsl:value-of
            select="replace(string-join((tokenize(translate(translate(substring-after(., '('), '|,', '  '), '?()*+', ''), '\s+')), ' | '), '#PCDATA', 'text()')"/>
          <xsl:text>)</xsl:text>
        </xsl:variable>
        <xsl:attribute name="test" select="$allowed-test"/>
        <xsl:value-of select="$element"/>
        <xsl:text> cannot contain the following elements: </xsl:text>
        <value-of select="{concat('(', $allowed-test, ')/name()')}"/>
      </sch:report>
      ...
      <xsl:if test="not(ends-with(., '*')) and contains(., ',')">
        <sch:report>
          <xsl:attribute name="test">
            ...
            <xsl:value-of
              select="string-join((tokenize($order-test, ' or ')[position() != last()]), ' or ')"/>
          </xsl:attribute>
          <xsl:text>Child elements of </xsl:text>
          <xsl:value-of select="$element"/>
          <xsl:text> are out of order. <!--The correct order is: </xsl:text><xsl:value-of select="replace(replace(replace(replace(replace(., '\|', ' or '), ',', ', then: '), '\*', '[0 or more]'), '\+', '[1 or more]'), '\?', '[0 or 1]')"/>-->Element model: </xsl:text>
        ...
        <xsl:choose>
          ...
          <xsl:when test="starts-with($str, '(')">
            ...
            <xsl:choose>
              ...
              <xsl:when test="ends-with($inside-paren, '*')">
                <xsl:call-template name="model-entities">
                  <xsl:with-param name="str"
                    select="substring-after(substring($str, string-length($inside-paren)), ',')"/>
              ...
              <xsl:when test="starts-with($inside-paren, '(') and 
                              contains(translate($inside-paren, '+?*', ''), ')|') and 
                              not(contains($inside-paren, ',('))">
                <xsl:variable name="or"
                  select="substring-after(substring-after($inside-paren, ')'),'|')"/>
                ...
                <xsl:if
                  test="not(matches(translate($either, '()?,+*', ''), translate($or, '()?,+*', ''))) and not(matches(translate($or, '()?,+*', ''), translate($either, '()?,+*', '')))">
                  <sch:assert>
                    <xsl:attribute name="test">
                      <xsl:text>if (</xsl:text>
                      <xsl:value-of
                        select="string-join((tokenize(translate($or, '?+*()', ''), ',')),' or ')"/>
                      <xsl:text>) then not(</xsl:text>
                      <xsl:value-of
                        select="string-join((tokenize(translate($either, '?+*()', ''), ',')),' or ')"/>
                    ...
                    <xsl:value-of
                      select="string-join((tokenize(string-join((tokenize(translate($either, '?+*()', ''), ',')),'&gt; or &lt;'), '\|')),'&gt; or &lt;')"/>
                    <xsl:text>&gt;) and (&lt;</xsl:text>
                    <xsl:value-of
                      select="string-join((tokenize(string-join((tokenize(translate($or, '?+*()', ''), ',')),'&gt; or &lt;'), '\|')),'&gt; or &lt;')"/>
                    <xsl:text>&gt;)</xsl:text>
                  </sch:assert>
                </xsl:if>
                <xsl:variable name="element" select="concat('&lt;',@name,'&gt;')"/>
                <xsl:choose>
                  ...
                  <xsl:when test="ends-with($inside-paren, '+')">
                    ...
                    <xsl:comment>
                      <xsl:text>Either/Or: </xsl:text>
                      <xsl:copy-of select="$inside-paren"/>
                    </xsl:comment>
                    <xsl:comment>
                      <xsl:text>Either: </xsl:text>
                      <xsl:copy-of select="$either"/>
                    </xsl:comment>
                    <xsl:comment>
                      <xsl:text>Or: </xsl:text>
                    ...
                    <sch:assert>
                      <xsl:attribute name="test">
                        <xsl:value-of
                          select="concat('(', string-join((tokenize(translate($either, '?+*()', ''), ',')),' or '), ')')"/>
                        <xsl:text> or </xsl:text>
                        <xsl:value-of
                          select="concat('(', string-join((tokenize(translate($or, '?+*()', ''), ',')),' or '), ')')"
                        />
                      </xsl:attribute>
                      <xsl:value-of select="$element"/>
                      <xsl:text> must contain at least one (&lt;</xsl:text>
                      <xsl:value-of
                        select="string-join((tokenize(string-join((tokenize(translate($either, '?+*()', ''), ',')),'&gt; or &lt;'), '\|')),'&gt; or &lt;')"/>
                      <xsl:text>&gt;) or (&lt;</xsl:text>
                      <xsl:value-of
                  ...
                  <xsl:otherwise>
                    ...
                    <xsl:for-each-group select="tokenize(translate($either, '()', ''), ',')"
                      group-by=".">
                      ...
                      <xsl:choose>
                        ...
                        <xsl:when test="ends-with(current-grouping-key(), '+')">
                          <sch:assert>
                            ...
                            <xsl:text> or &lt;</xsl:text>
                            <xsl:value-of
                              select="string-join((tokenize(string-join((tokenize(translate($or, '?+*()', ''), ',')),'&gt;/&lt;'), '\|')),'&gt;/&lt;')"/>
                            <xsl:text>&gt;</xsl:text>
                          </sch:assert>
                        </xsl:when>
                          ...
                          <sch:assert>
                            <xsl:attribute name="test">
                              ...
                              <xsl:choose>
                                <xsl:when test="not(contains($or, ',') or contains($or, '|'))">
                                  <xsl:if
                                    test="not(contains($or, '?') or contains($or, '+') or contains($or, '*'))">
                                    <xsl:value-of select="translate($or, '~() ', '')"/>
                                  </xsl:if>
                                </xsl:when>
                                <xsl:otherwise>
                    ...
                    <xsl:for-each-group select="tokenize(translate($or, '()', ''), ',')"
                      group-by=".">
                      <xsl:variable name="entity-name"
                        select="translate(current-grouping-key(), '+?*~ ', '')"/>
                      <xsl:choose>
                            ...
                            <xsl:when test="not(contains($either, '+') or contains($either, '?'))">
                              <sch:assert>
                                <xsl:attribute name="test">
                                  ...
                                  <xsl:choose>
                                    <xsl:when
                                      test="not(contains($either, ',') or contains($either, '|'))">
                                ...
                                <xsl:text> or &lt;</xsl:text>
                                <xsl:value-of
                                  select="string-join((tokenize(string-join((tokenize(translate($either, '?+*()', ''), ',')),'&gt;/&lt;'), '\|')),'&gt;/&lt;')"/>
                                <xsl:text>&gt;</xsl:text>
                              </sch:assert>
                            </xsl:when>
                          </xsl:choose>
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="ends-with(translate($entity, ')', ''), '+')">
                    <sch:assert>
                      <xsl:attribute name="test">
                        <xsl:value-of
                          select="string-join((tokenize(translate($entity, '?+*()', ''), '\|')), ' or ')"
                        />
                      </xsl:attribute>
                      <xsl:value-of select="concat('&lt;',@name,'&gt;')"/>
                  ...
                  <xsl:when test="ends-with(translate($entity, ')', ''), '?')">
                    <sch:report>
                      <xsl:attribute name="test">
                        <xsl:value-of
                          select="concat('count(',string-join((tokenize(translate($entity, '?+*()', ''), '\|')), ' or '), ') > 1')"
                        />
                      </xsl:attribute>
                      <xsl:value-of select="concat('&lt;',@name,'&gt;')"/>
                  ...
                  <xsl:otherwise>
                    <sch:assert>
                      <xsl:attribute name="test">
                        <xsl:text>if(</xsl:text>
                        <xsl:value-of
                          select="translate(substring-before($entity, '|'), '(),*? |+~','')"/>
                        <xsl:text>) then not(</xsl:text>
                        <xsl:value-of
                          select="translate(substring-after($entity, '|'), '(),*? |+~','')"/>
                        <xsl:text>) else not(</xsl:text>
                        <xsl:value-of
                          select="translate(substring-before($entity, '|'), '(),*? |+~','')"/>
                        <xsl:text>)</xsl:text>
                      ...
                      <xsl:value-of
                        select="concat('&lt;',translate(substring-before($entity, '|'), '(),*? |+~',''),'&gt;')"/>
                      <xsl:text> and </xsl:text>
                      <xsl:value-of
                        select="concat('&lt;',translate(substring-after($entity, '|'), '(),*? |+~',''),'&gt;')"
                      />
                    </sch:assert>
                    <xsl:if test="not(contains($entity, '*'))">
                      ...
                      <sch:report>
                        <xsl:attribute name="test">
                          <xsl:value-of
                            select="concat('count(', translate(substring-before($entity, '|'), '(),*? |+~',''), ') > 1')"/>
                          <xsl:text> or </xsl:text>
                          <xsl:value-of
                            select="concat('count(', translate(substring-after($entity, '|'), '(),*? |+~',''), ') > 1')"
                          />
                        </xsl:attribute>
                        <xsl:value-of select="concat('&lt;',@name,'&gt;')"/>
                        <xsl:text> cannot have more than one </xsl:text>
            ...
            <xsl:call-template name="ordering">
              ...
              <xsl:with-param name="output">
                ...
                <xsl:for-each-group select="tokenize($entity, '\|')" group-by=".">
                  ...
                  <xsl:value-of
                    select="string-join((tokenize(translate(translate(substring-after($str,','), '+?*()~ ', ''), '|,', '  '), ' ')), ' or preceding-sibling::')"/>
                  <xsl:text>] or </xsl:text>
                </xsl:for-each-group>
              </xsl:with-param>
            </xsl:call-template>
  ...
  <xsl:template name="paren-nesting">
    ...
    <xsl:param name="level"
      select="string-length(substring-before($str, ')')) - string-length(translate(substring-before($str, ')'), '(', ''))"/>
    <xsl:choose>
      ...
      <xsl:when test="$level=2">
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <xsl:value-of select="substring-after($content, '(')"/>
            <xsl:value-of
              select="translate(substring($str, string-length($content) + 1, 2), '|,', '~')"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:when test="$level=3">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after($str, ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
      ...
      <xsl:when test="$level=4">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after(substring-after($str, ')'), ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
          ...
          <xsl:otherwise>
            <xsl:variable name="content"
              select="string-join(( substring-before($str, ')'), 
                        	substring-before(substring-after($str, ')'), ')'),
                        	substring-before(substring-after(substring-after($str, ')'), ')'), ')'),
      ...
      <xsl:when test="$level=5">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after(substring-after(substring-after($str, ')'), ')'), ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
      ...
      <xsl:when test="$level=6">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after(substring-after(substring-after(substring-after($str, ')'), ')'), ')'), ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
      ...
      <xsl:when test="$level=7">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after($str, ')'), ')'), ')'), ')'), ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
      ...
      <xsl:when test="$level=8">
        <xsl:choose>
          <xsl:when
            test="contains(substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after($str, ')'), ')'), ')'), ')'), ')'), ')'), ')'), ')'), '(')">
            <xsl:call-template name="paren-nesting">
              <xsl:with-param name="str" select="$str"/>
              <xsl:with-param name="level" select="$level+1"/>
            </xsl:call-template>
