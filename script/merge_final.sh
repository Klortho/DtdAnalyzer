...
cd source
    ...
    echo '<final-list>' > $datapath/source/short_elements.xml
    
    echo 'Grab Green DTDs'
    find . -name "*short.arch.out*" > arch.txt

    echo 'Grab Blue DTDs'
    find . -name "*short.pub.out*" > pub.txt

    echo 'Grab Pumpkin DTDs'
    find . -name "nlm2.1.short.auth.out*" > 21.txt
    find . -name "nlm2.2.short.auth.out*" > 22.txt
    find . -name "nlm2.3.short.auth.out*" > 23.txt
    find . -name "nlm3.0.short.auth.out*" > 30.txt
    find . -name "jats1.0.short.auth.out*" > j10.txt
    find . -name "jats1.1d1.short.auth.out*" > j1d1.txt
    find . -name "jats1.1d2.short.auth.out*" > j1d2.txt
    find . -name "jats1.1d3.short.auth.out*" > j1d3.txt
    find . -name "jats1.1.short.auth.out*" > j11.txt
...
if [ -e short_elements.xml ]

then
echo "Converting XML file to HTML"
java -jar /pmc/JAVA/saxon9b/saxon9.jar -xsl:$xslpath/final_element_list.xsl -s:short_elements.xml -o:short_elements.html
